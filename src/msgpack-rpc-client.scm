                               ...
                               (with-lock req-table-lock
                                          (hash-table-set! req-table id
                                                           (if (null? err)
                                                               (cons #:success result)
                                                               (cons #:error err))))))
                            (else (error (format "Unrecognized message type ~a." (car msg)))))) ; TODO
        ...
        (let ((connect #f)
              ...
              (call  ; send a request or notification and return the id
                (lambda (mode method args)
                  (logger "aquiring the output lock for calling")
                  (with-lock out-lock
                             ...
                             (case mode
                               ...
                               (else
                                 (error
                                   (format "Unrecognized message type ~a." mode)))))))
              ...
              (bind  ; register a method
                (lambda (method-name method)
                  ...
                  (logger (format "Method ~a bound." method-name))))
               ...
               (set! connect
                 (lambda ()
                   (let-values (((l-in l-out) (tcp-connect host port)))
                     (logger "TCP connection initialized.")
                     (set! in l-in)
                     (set! out l-out)
                     #t)))))
            ((#:unix)
                 ...
                 (lambda ()
                   (let ((sock (socket af/unix
                     ...
                     (let-values (((s-in s-out) (socket-i/o-ports sock)))
                       (logger "UNIX socket connection initialized.")
                       (set! in s-in)
                       (set! out s-out)
                       #t))))))
            ((#:extend)
             ...
             (set! connect
               (lambda ()
                 (let-values (((l-in l-out) ((car args))))
                   (logger "Extended connection initialized.")
                   (set! in l-in)
                   (set! out l-out)))))
            (else
